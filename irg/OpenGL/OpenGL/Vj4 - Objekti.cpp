...
int main(int argc, char *argv[]) {
	...
	ucitajObjekt("C:\\Users\\kuzmi\\Downloads\\faks\\IRG\\labosi\\teddy.obj");
	boja.push_back(0.1);
	boja.push_back(0.2);
	boja.push_back(0.3);
	boja.push_back(0.4);
...
void CrtajPoligonKonv() {
	...
	for (int j = 0; j < poljePolygona.size(); j++) {
		...
		for (int i = 0; i < 3; i++) {
			// teddy    //
			bresenham_nacrtaj_cjelobrojni(tocke.at(i0).x * 10 + 689, tocke.at(i0).y*10 + 300, tocke.at(i).x * 10 + 689, tocke.at(i).y * 10 + 300);
